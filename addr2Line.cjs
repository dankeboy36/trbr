...
// node addr2Line.cjs /Users/kittaakos/Downloads/stack_elf/crash_PPM_to_ESP_NOW_ESP32C3_a.ino.elf /Users/kittaakos/Library/Arduino15/packages/esp32/tools/riscv32-esp-elf-gdb/14.2_20240403/bin/riscv32-esp-elf-gdb /Users/kittaakos/Downloads/stack_elf/registers_stack.txt
...
const [, , elfPath, toolPath, inputFile] = process.argv

if (!elfPath || !toolPath || !inputFile) {
  console.error(
    'Usage: node symbolicate-panic.js <elfFile> <toolPath> <panicOutput.txt>'
  )
  process.exit(1)
}

const panicText = fs.readFileSync(inputFile, 'utf8')

// Extract addresses that look like `0x403813ea` or `#1  0x403872b2`
const addressPattern = /0x[0-9a-fA-F]+/g
const addresses = [...new Set(panicText.match(addressPattern) || [])]

console.log(`Found ${addresses.length} unique addresses.`)

// For each address, create a temporary GDB script, run GDB, parse and print the result.

function toHexString(addr) {
...
const gdbArgs = buildAddr2LineFlags(addresses, elfPath)
const result = spawnSync(toolPath, gdbArgs)
console.error('GDB command:', toolPath, gdbArgs.join(' '))
console.error('GDB stdout:', result.stdout?.toString() ?? '')
console.error('GDB stderr:', result.stderr?.toString() ?? '')
const rawOutput = result.stdout?.toString() ?? ''
const sections = rawOutput.split(/>>> ADDR: (0x[0-9a-fA-F]+)[^\n]*\n/).slice(1)
...
for (let i = 0; i < sections.length; i += 2) {
  ...
  const lineInfoMatch = lines.find((l) =>
    /Line \d+ of ".*?" starts at address/.test(l)
  )
  if (lineInfoMatch) {
    const lineMatch = lineInfoMatch.match(/Line (\d+) of "(.*?)"/)
    const startAddrMatch = lineInfoMatch.match(
      /starts at address (0x[0-9a-fA-F]+)/
    )
    const symbolMatch = lineInfoMatch.match(/<([^>]+)>/)
    if (lineMatch && startAddrMatch && symbolMatch) {
      infoLine = `${symbolMatch[1]} at ${lineMatch[2]}:${lineMatch[1]}`
    }
  }
  ...
  const match = lines
    ...
    ?.match(/<([^>]+)>/)
  let symbolLine = match ? match[1] : '??'

  const fileLineRaw =
    lines.find((l) => l.includes('Line') && l.includes('.c')) || ''
  const fileLineMatch = fileLineRaw.match(/Line (\d+) of "(.*?)"/)
  const fileLine = fileLineMatch
    ? `${fileLineMatch[2]}:${fileLineMatch[1]}`
    : '??:0'

  const isUnresolved = !symbolLine || symbolLine === '??'
  const romSymbol = esp32c3RomLd[addr.toLowerCase()]
  if (romSymbol) {
    symbolLine = isUnresolved ? `${romSymbol} (ROM)` : symbolLine
  }

  const symbolResolved = symbolLine !== '??'
  const fileResolved = fileLine !== '??:0'

  // New merged/enriched logic for header and mergedLine
  const fullMatch = lineInfoMatch?.match(
    /Line (\d+) of "(.*?)" starts at address (0x[0-9a-fA-F]+) <([^>]+)> and ends at (0x[0-9a-fA-F]+) <([^>]+)>/
  )
  const mergedLine = fullMatch
    ? `${fullMatch[4]} [${fullMatch[3]}..${fullMatch[5]}] at ${fullMatch[2]}:${fullMatch[1]}`
    : symbolResolved && fileResolved
    ? infoLine
    : `${symbolLine.trim()} at ${fileLine.trim()}`
  const header = `#${i / 2}  ${addr} in ${mergedLine}`
  console.log(header)
}
