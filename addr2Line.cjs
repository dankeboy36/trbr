...
for (let i = 0; i < sections.length; i += 2) {
  ...
  if (isUnresolved && esp32c3RomLd[addr.toLowerCase()]) {
    symbolLine = esp32c3RomLd[addr.toLowerCase()] + ' (ROM)'
  }

  const disasm = lines.find((l) => /^\s*=>\s/.test(l))
  console.log(
    `#${i / 2}  ${addr} in ${symbolLine.trim()} at ${fileLine.trim()}`
  )
  if (disasm) {
    console.log(`        ; ${disasm.trim()}`)
  }
}
